...
#define savePath "E:\\Web\\experiment3\\Client2Server\\Client2ServerSR.txt"
#define endure 5
...
BOOL ack[SEQ_SIZE];                 //序列号可用情况，对应0~19的ack
BOOL Received[SEQ_SIZE];            //收到ack情况，对应0~19的ack
int curSeq;                         //当前数据包的seq 
int curAck;                         //当前等待确认的ack 
int totalSeq;                       //收到的包的总数 
int totalPacket;                    //需要发送的包总数
int totalACK;                       //收到总共的ACK数
...
int main(int argc, char* argv[]) { 
   ...
   //将测试数据读入内存 
   ifstream icin; 
   icin.open("E:\\Web\\experiment3\\test.txt"); 
   char data[1024 * 113]; 
   ...
   while(true){ 
         ...
         while(runFlag){ 
            switch(stage){ 
               ...
               case 3://数据传输阶段
                  ...
                  while(ack[curAck] && Received[curAck]){
                     Received[curAck] = FALSE;
      ...
      //用户输入-upload
            ...
            int totalBytes = 0;
            int expectedBaseSeq = 1;
            BOOL received[SEQ_SIZE + 1];
            char cache[1024 * 113];
